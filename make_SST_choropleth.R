#' @title Function for plotting SST chloropleth map for Puget Sound
...
#' @details OUTPUT: Chloropleth maps of SST for long-term and short-term projections
#' @details used code from https://github.com/hmorzaria/gommesopelagic/blob/master/make_map.R
#' @description Shows SST projections for 2020-2050 and 2070-2100 under multiple models and scenarios
...
make_SST_chloropleth <- function(data.downscaled.table, model.type, experiment.type, plot.filename1, plot.filename2, combined.filename, save.path){
...
SSTmodel.map1 <- ggplot(data = world) +
  ...
  scale_fill_distiller(palette = "Reds", direction = 1,limits=c(10.73,16.65)) +
  theme(
