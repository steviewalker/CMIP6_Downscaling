#' @title Plotting Interpolated SST Anomalies for Salish Sea
...
plot_SSTanomaly <- function(this.file, data.path, save.path) {
  
  print(this.file)
  setwd(data.path)
  
  #this.file <- "tos_Omon_CNRM-CM6-1-HR_ssp245_r1i1p1f2_anomaly_202001-205012PugetSound.asc"
  ...
  ggplot(data=df, aes(y=Latitude, x=Longitude, color="red")) +
    ...
    labs(title = plot.title,
         fill = "SST anomaly") +
    scale_fill_distiller(palette = "RdBu", direction = -1, limits=c(0.5,4.7)) +
    #scale_fill_gradient(low = "#4393c3", high = "#b30000", limits=c(0.5,4.7)) +
