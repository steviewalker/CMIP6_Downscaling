#' @title Plotting SST chloropleth map for Puget Sound
...
#' @details OUTPUT: Chloropleth map of SST for long-term and short-term projections
#' @details used code from https://github.com/hmorzaria/gommesopelagic/blob/master/make_map.R
#' @description Shows SST projections for 2020-2050 and 2070-2100 under the GFDL-CM4 model ssp585 scenario
...
make_chloropleth(data.downscaled.table, model.type = "GFDL-CM4", experiment.type ="ssp585", 
...
make_chloropleth <- function(data.downscaled.table, model.type, experiment.type, plot.filename1, plot.filename2, combined.filename, save.path){
...
box.SST.grouping.subset <- box.SST.grouping %>%
...
#changing back to save directory
...
#correcting Atlantis polygon ids
polygon1 <-  box.SST.grouping.subset %>%
  mutate(id=as.character(id)) %>% 
  right_join(model.shape.df, by="id")

#plotting figure
SSTmodel.map1 <- ggplot(data = world) +
  geom_sf() +
  ...
  scale_fill_distiller(palette = "Reds", direction = 1,limits=c(11,18)) +
  theme(
    axis.title.y = element_blank(),
    axis.title.x = element_blank()
  )

#saving long term map
ggsave(SSTmodel.map1, filename = plot.filename1, device = "png", path = save.path, 
...
#subsetting boxes for one scenario, model, and variable
box.SST.grouping.subset2 <- box.SST.grouping %>%
  filter(model == "GFDL-CM4" & experiment == "ssp585" & term == "Short-term") %>%
  dplyr::select(boxIndex,mean.roms_cimp6) %>%
  dplyr::rename(id=boxIndex)

#correcting Atlantis polygon ids
polygon2 <-  box.SST.grouping.subset2 %>%
  mutate(id=as.character(id)) %>% 
  right_join(model.shape.df, by="id")

#plotting figure
SSTmodel.map2 <- ggplot(data = world) +
  geom_sf() +
  annotation_scale(location = "bl", width_hint = 0.5,
                   height = unit(0.095, "in"), line_width = 0.6) +
  annotation_north_arrow(location = "br", which_north = "true", 
  ...
  theme(
    axis.title.y = element_blank(),
    axis.title.x = element_blank(),
    legend.position = "none" #uncomment this line when creating facet grid
  )

#saving short term figure
ggsave(SSTmodel.map2, filename = plot.filename2, device = "png", path = save.path, 
       dpi = 150, width = 13, height = 11, units = "cm")

#combining short-term and long-term into one grid
grid.combine <- grid.arrange(SSTmodel.map2,SSTmodel.map1, ncol =2, nrow=1)
